...
#+END_SRC
*** Window management
Two built-in minor modes allow me to recover a lost window config (winner-mode) and move across windows with Shift-<Arrow>, which is quite convenient.
#+BEGIN_SRC emacs-lisp
  (winner-mode)
...
** Discovering emacs
*** Icicles
With icicles, S-TAB will let you know what key bindings are available at any given moment. Pretty awesome. And that's just scratching the surface of what icicle does...
#+BEGIN_SRC emacs-lisp
(ido-mode nil)
(use-package icicles
  :init
  ...
  :bind (("C-x C-m" . icicle-execute-extended-command)))
#+END_SRC
...
But sometimes you need a directory tree.
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (use-package web-mode
    :mode "\\.mustache$" "\\.html$" "\\.erb$" "\\.jsx$" "\\.eex$"
    :config
...
(use-package ox-reveal :config (use-package htmlize))
#+END_SRC
** Version control
*** Git
...
(use-package railscasts-theme :defer t)
#+END_SRC
